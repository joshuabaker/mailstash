...
import type { IDBEmail, IDBAttachment, IDBAttachmentBlob } from "./idb.js";

export async function parseEmailForIDB(
  rawEmail: string,
  accountId: string,
): Promise<IDBEmail> {
  ...
  const attachmentBlobs: IDBAttachmentBlob[] = [];
  ...
  const emlBytes = new TextEncoder().encode(rawEmail).buffer as ArrayBuffer;
